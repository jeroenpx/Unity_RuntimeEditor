...
namespace Battlehub.UIControls.XR
{
    /// <summary>
    /// This class is copy of TrackedDeviceGraphicRaycaster from XR Interaction Toolkit 0.9.4. 
    /// Required to fix SortedRaycastGraphics -> if (!graphic.raycastTarget) added
    /// and to expose m_BlockingMask as public property
    /// </summary>
    public class GraphicsRaycasterXR : BaseRaycaster
    {
        ...
        private void SortedRaycastGraphics(Canvas canvas, Ray ray, float maxDistance, LayerMask layerMask, List<RaycastHitData> results)
        {
            ...
            Sort(s_SortedGraphics, m_raycastHitComparer);
        ...
        private static void Sort<T>(IList<T> hits, IComparer<T> comparer) where T : struct
        {
            T temp;
