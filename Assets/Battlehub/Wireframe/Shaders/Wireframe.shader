	...
	SubShader
	{
		...
		Pass
		{
			...
			float edgeFactor(float3 barycentric) {
				...
				return min(a3.z,  a3.x); //min(min(a3.x, a3.y), a3.z);
			}

			fixed4 frag(v2f i) : SV_Target
			{
				float4 color = i.color;
				color.a = 1.0 - step(1.0 - edgeFactor(i.baryc), 0); // 1.0 - edgeFactor(i.baryc); //lerp(1.0 - edgeFactor(i.baryc), 0, color.a);
		...
		Pass
		{
			...
			{
				float4 color = i.color;
				color.a = 1.0 - step(1.0 - edgeFactor(i.baryc), 0); //lerp( 1.0 - edgeFactor(i.baryc), 0, color.a);
				return color * tex2D(_MainTex, i.uv);
			}
				ENDCG
		}
